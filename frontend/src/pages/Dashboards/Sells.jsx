...
import { useGetUserSellsQuery } from "../../redux/api/order";
        ...
        <table className="container table-auto">
          <thead className="w-full border ">
            <tr className="mb-[5rem] border-b border-yellow-800">
              ...
              <th className="text-center text-yellow-500 hidden md:table-cell pl-2">BUYER</th>
          ...
          <tbody className="text-sm font-thin md:font-light md:text-lg">
            {orders.map((order) => (
              <tr key={order?._id} >
                ...
                <td className="text-center  hidden md:table-cell pl-2 underline hover:text-sky-300 text-violet-400"><Link to={`/${order?.user?._id}/profile`} >@{order?.user.username}</Link></td>
